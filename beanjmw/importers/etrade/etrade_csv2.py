...
# these are the actual columns in the CSV/TXT file
etrade_cols = ["Activity/Trade Date","Transaction Date","Settlement Date","Activity Type","Description","Symbol","Cusip","Quantity #","Price $","Amount $","Commission","Category","Note"]


# map to universal row
# Not all CSV cols are mapped in the universal row (only matching keys)
# BUT all columns in csv cols above must be in this map as a key
etrade_map_cols = {
'Activity/Trade Date':'trade_date',
...
class Importer(ImporterProtocol):
	...
	def identify(self, file):
		...
		if os.path.splitext(file.name)[1].upper()=='.CSV':
			...
			while ln < len(head_lines):
				if 'Account Activity for Individual Brokerage' in head_lines[ln]:
					toks=head_lines[ln].split('-')[1].split(' ')
					if len(toks) > 1:
						fa=toks[0]
	...
	def extract(self, file, existing_entries=None):
		...
		try:
			...
			sys.stderr.write("etrade_csv2: Unable to open or parse {0}".format(file.name))
			return([])
	...
	def map_actions(self,urd):
		if urd['action'] in action_map:
			...
			sys.stderr.write("etrade_csv2: Unknown inv action: {0} in {1}\n".format(urd['action'],urd))
		...
		if urd['action']=='Div':
			if urd['quantity']!=0 and urd['quantity']!=None:
				...
				try:
					...
					sys.stderr.write('etrade_csv2: problem with conversion {0}\n'.format(urd))
#					breakpoint()
	...
	def map_universal_transactions(self,rows):
		...
		for row in rows:
			...
			# NOTE: Etrade sometimes downloads the symbol as '--' even for 
			# some valid transactions - these must be hand edited in the 
			# downloaded file!
			if len(urd['symbol']) == 0 or '#' in urd['symbol'] or urd['symbol']=='--':
	...
	def create_table(self,lines):
		...
		if not is_etrade or len(cols)!=len(etrade_cols):
			sys.stderr.write("etrade_csv2: Bad format {0}".format(cols))
			return(table)
